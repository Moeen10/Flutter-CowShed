...
class VaccineSelectionDialog extends StatefulWidget {
  final List<VACCINE> availableChips;
  final List<VACCINE> selectedChips;
  final COW? cow;
  final void Function(List<VACCINE>) onChipSelectionChanged;

  const VaccineSelectionDialog({
    Key? key,
  ...
  @override
  State<VaccineSelectionDialog> createState() => _VaccineSelectionDialogState();
}

class _VaccineSelectionDialogState extends State<VaccineSelectionDialog> {

  List<VACCINE> _selectedChips = [];
  List<String> vaccine_name = [];
  @override
  void initState() {
    super.initState();
  ...
  void _handleChipSelection(VACCINE chipValue) {
    print("*****************");
    print(chipValue.vaccineName);
    setState(() {
      if (_selectedChips.contains(chipValue)) {
        _selectedChips.remove(chipValue);

      } else {
        vaccine_name.add(chipValue.vaccineName!);
  ...
  Color _getChipColor(VACCINE chipValue) {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: Text('Select Vaccine for ${widget.cow?.cattleId}'),
      content: SingleChildScrollView(
        child: Wrap(
          children: widget.availableChips.map((chipValue) => ChoiceChip(
            label: Text('${chipValue.vaccineName}' ),
  ...
  void submitData() async {
    ...
    print("SUBMIT From Update Vaccine");
    widget.cow?.vaccine = vaccine_name;
    int len  =widget.cow!.vaccine?.length ?? 0;
    for(int i = 0;i<len;i++){
      String? name = widget.cow?.vaccine![i];
      widget.cow?.vaccine![i]=   utf8.decode(name!.runes.toList());
    }
    // print("Disease : ");
    // for(int i = 0;i<len;i++){
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print("141 ERROR FROM ELSE");
        PopupMessage(context,"Error",responseMap['message']);
        print(response.statusCode);
      }
    } catch (e) {
      print("146  ERROR FROM Catch");
